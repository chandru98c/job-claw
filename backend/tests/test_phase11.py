...
async def test_duplicate_application_prevention(client, async_db_session):
    ...
    # Create application
    resp = await client.post("/applications", json={"job_id": job_id}, params={"profile_id": profile_id})
    assert resp.status_code == 201
    
    # Create again -> conflict
    resp2 = await client.post("/applications", json={"job_id": job_id}, params={"profile_id": profile_id})
...
async def test_authorization_checks(client, async_db_session):
    ...
    resp2 = await client.get(f"/applications/{app_id}?profile_id={profile2_id}")
    assert resp2.status_code == 403
    
    # Cannot approve someone else's app
    resp3 = await client.post(f"/applications/{app_id}/approve?profile_id={profile2_id}", json={})
...
async def test_approval_flow_state_machine(client, async_db_session):
    ...
    resp2 = await client.post(f"/applications/{app_id}/approve", params={"profile_id": profile_id}, json={})
    ...
    # Now can approve
    resp3 = await client.post(f"/applications/{app_id}/approve", params={"profile_id": profile_id}, json={})
    ...
    resp4 = await client.post(f"/applications/{app_id}/submit", params={"profile_id": profile_id})
